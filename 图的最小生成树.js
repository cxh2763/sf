...
function getMinDisNode(pointSet,distance,nowPointSet){
    ...
    for(var i = 0;i < nowPointSet.length;i++){
        var nowPointIndex = getIndex(nowPointSet[i].value,pointSet);//获取当前节点的序号
...
function prim(pointSet,distance,start){
    var newPonitSet = [];
    newPonitSet.push(start);
    //获取最小代价的边
    while(true){
        var minDisNode = getMinDisNode(pointSet,distance,newPonitSet);
        newPonitSet.push(minDisNode);
        if(newPonitSet.length == pointSet.length){
            break;
        }
    }
}
